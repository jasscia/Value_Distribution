...
export function ProjectList(props){
    ...
    return props.projectList.map((item)=>
        <ProjectItem item={item} key={item.id}
            handleIfShowCreatePage={props.handleIfShowCreatePage}
...
function ProjectItem(props){
    ...
    return <div className="item">                   
                <div className="projectName">
                    ...
                    <span className="edit" 
                        onClick={()=>{props.handleIfShowCreatePage();
                                    props.handleActiveItem(props.item)}}>edit</span>
                </div>
                <div className="projectBaseInfo"> 
                    <div className="projectWorth">合同额：$<em>{props.item.worth}</em>&ensp;万</div>
                    <div className="projectDays">工期：<em>{props.item.day_num}</em>&ensp;天</div>
                </div>
